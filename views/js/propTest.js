...
let state2_init = '2022-12-12T10:46:32.816   6.900000000000000e+03   0.000000000000000e+00   0.000000000000000e+00   0.000000000000000e+00  -1.057790461084519e+00   7.526570219427627e+00'.split(/ +/)
...
let state2_final = '2022-12-13T10:46:32.816   3.748684147875897e+03  -7.392369513902674e+02   5.740869357598748e+03  -6.379081281614028e+00  -6.894912833553686e-01   4.068334979131852e+00'.split(/ +/)
...
function highPrecisionProp(position = [42164, 0, 0, 0, 3.074, 0], date = new Date()) {
    ...
    let a_pert = recursiveGeoPotential(position, 8, date)
    a = math.add(a_pert.a,a)
    a_pert = thirdBodyEffects(position, date)
    a = math.add(a_pert,a)
    a_pert = atmosphericDragEffect(position, options = {})
    a = math.add(a_pert,a)
    return [
...
function atmosphericDragEffect(eciState = state2_init, options = {}) {
    let {cd = 2.2, m = 850, area = 15} = options
    let re = 6378.137
    let r = math.norm(eciState.slice(0,3))
    let h = r - re
    let rho = getAtmosphereDensity(h)
...
function getAtmosphereDensity(h = 747.2119) {
