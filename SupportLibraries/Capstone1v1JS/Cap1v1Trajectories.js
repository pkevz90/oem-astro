...
function calcData() {
	let curTime = Number(currentTime.value);
	let redR = posHistory.red[Math.floor(curTime*3600/calcDt)];
	let blueR = posHistory.blue[Math.floor(curTime*3600/calcDt)];
	setCurrentPoints(9,blueR,10,redR);
	dataSpans[0].textContent = math.norm([redR[0][0]-blueR[0][0],redR[1][0]-blueR[1][0]]).toFixed(2);
	...
	dataSpans[3].textContent = (catsAngle*180/Math.PI).toFixed(2);
	...
	dataSpans[1].textContent = math.norm([redR[0][0]-blueR[0][0],redR[1][0]-blueR[1][0]]).toFixed(2);
	dataSpans[2].textContent = (t2/3600).toFixed(1);
	globalChartRef.update();
}
...
function burnCalc(xMouse, yMouse, click) {
	if (click) {
		...
		if (chosenWaypoint[1] === 0) {
			...
			burnRows[(chosenWaypoint[0])*5+1].textContent = (distance/20*Math.sin(az)).toFixed(2);
			burnRows[(chosenWaypoint[0])*5+2].textContent =  (distance/20*Math.cos(az)).toFixed(2);
		}
		else {
			redBurns[chosenWaypoint[0]] = [distance/20*Math.sin(az), distance/20*Math.cos(az)];
			burns2waypoints(redInitState,redBurns,3,10800);
			burnRows[(chosenWaypoint[0])*5+3].textContent = (distance/20*Math.sin(az)).toFixed(2);
			burnRows[(chosenWaypoint[0])*5+4].textContent =  (distance/20*Math.cos(az)).toFixed(2);
		}
		// console.log('3')
